...
<%
HttpSession sesdr=request.getSession(false);
String dr=(String)sesdr.getAttribute("dr");
String date=(String)sesdr.getAttribute("date");
...
<html lang="en">
	...
	/>
	<sql:update dataSource="${cl_reg }"  var="update">
	update doctor_avail set status='busy' where name='<%=dr%>' && from1='<%=from1%>';
	</sql:update>
					...
					<div class="navbar-collapse collapse">
						<ul class="nav navbar-nav navbar-right">
							...
							<sql:query var="doc" dataSource="${cl_reg }">
								select * from doctor where user='<%=dr %>';  
							</sql:query>

							<c:forEach var="row" items="${doc.rows }">
							<c:set var="dd" value="${row.First_name } ${row.Last_name }" />
							</c:forEach>

							<sql:update dataSource="${cl_reg }">
							update doctor_avail set status='busy' where name='${dd }' and from1='<%=from1 %>';
							</sql:update>
							
							
							<sql:update dataSource="${cl_reg }">
							insert into appointment (patient,doctor,from1,to1,date) values ('<%=ss %>','<%=dr %>','<%=from1 %>','<%=to1 %>','<%=date %>');
							</sql:update>
	...
	<div class="row">
		...
		<div class="col-sm-8">
			<p>
				...
				<a href="index.jsp">Click Here</A> to goto Home Page.<br>
				<a href="history.jsp">Click Here</A> to goto History Page.
				</font>	
		...
		<sql:query dataSource="${cl_reg}" var="result12">
								select Email from patient where user="<%=ss%>";
		</sql:query>
		<c:forEach items="${result12.rows }" var="row">
		...
		<c:forEach items="${result123.rows }" var="row">
			<c:set var="name" value="${row.First_name} ${row.Last_name }"> </c:set>
		</c:forEach>

		<%
		
		String to=pageContext.getAttribute("email").toString();//change accordingly  
		String doc=pageContext.getAttribute("name").toString();
		...
		message.setSubject("Appointmant - Vision Medicare");  
		message.setText("Your appointment has been successfuly booked  \nDoctor name : "+doc+"\nDate : "+date+"\nFrom : "+from1+"\nTo : "+to1+"\nThank you");  
		...
		} catch (MessagingException e) {System.out.println(to+"error"+e.getMessage());}  
