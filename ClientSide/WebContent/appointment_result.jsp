					...
					<div class="row">
						<div class="col-md-12">
							...
							<div class="search-classic">
								...
								<hr>
								<sql:query var="appoint" dataSource="${cl_reg }">
								select * from appointment where patient='<%=ss %>';
								</sql:query>
								<c:forEach var="row" items="${appoint.rows }">
								<div class="search-result">
									<h4>
									<c:set var="dd" value="${row.doctor}" />
									<sql:query var="doc" dataSource="${cl_reg}">
									select * from doctor where user='<c:out value="${dd }"></c:out>';
									</sql:query>
									
									
									<a href="#">
									<c:forEach var="doct" items="${doc.rows}">
									<c:set value="${doct.First_name} ${doct.Last_name }" var="doctr" />
									</c:forEach>
										<b>Doctor : <c:out value="${doctr}" /></b> 
									
									</a></h4>
									<a href="#">
										<b>Date : <c:out value="${row.date}" /></b>
									</a>
									<p>
									From :: <c:out value="${row.from1 }"></c:out> -- To :: <c:out value="${row.to1 }"></c:out>
