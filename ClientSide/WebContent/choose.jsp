...
String s1=(String)sesdr.getAttribute("dr");
String date=request.getParameter("date");
sesdr.setAttribute("date",date);
%>
<sql:setDataSource var="cl_reg1" driver="com.mysql.jdbc.Driver"
    url="jdbc:mysql://localhost:3306/admin_hospital"
    user="root"  password="haresh"/>
<sql:query var="doc" dataSource="${cl_reg1 }">
...
<sql:query dataSource="${cl_reg1}" var="result1">
select * from doctor_avail where name='${dd}' and date='<%=date%>';
				...
				<div class="row">
					...
					<table class="table table-hover" id="sample-table-1">
						...
						<c:forEach var="row" items="${result1.rows}">
							<form action=appoint.jsp>	
						<tr>
							<td >
									<input type="text" value="<c:out value='${row.from1}' />" name="from" >	
							</td>
							
							<td>
								<input type="text" value="<c:out value='${row.to1}' />" name="to" >
							</td>
							
							<td>
								<c:if test="${row.status == 'Free' }">
							
								
									<input type="submit" class="btn btn-success" Name="submit" value=" Book " >
