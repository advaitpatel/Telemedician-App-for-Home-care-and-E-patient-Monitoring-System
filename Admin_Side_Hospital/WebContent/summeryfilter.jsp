...
ResultSet rs;
String ss,ss1;
int appointcount=0,doccount=0,patcount=0,staffcount=0,storecount=0,labcount=0;
	try
	{
		Class.forName("com.mysql.jdbc.Driver");
		con=DriverManager.getConnection("jdbc:mysql://localhost:3306/admin_hospital","root","haresh");
		st=con.createStatement();
		rs=st.executeQuery("select date from appointment");
		while(rs.next())
		{
		...
		rs=st.executeQuery("select regdate from patient");
		while(rs.next())
		{
			ss=rs.getString("regdate");
			Calendar newcal=Calendar.getInstance();
			newcal.setTime(sdf.parse(ss));
			if(now.after(newcal))
			{
		...
		rs=st.executeQuery("select regdate from doctor");
		while(rs.next())
		{
			ss=rs.getString("date");
			Calendar newcal=Calendar.getInstance();
		...
		rs=st.executeQuery("select regdate from staff");
		while(rs.next())
		{
			ss=rs.getString("regdate");
			Calendar newcal=Calendar.getInstance();
		...
		rs=st.executeQuery("select regdate from medical_store");
		while(rs.next())
		{
			ss=rs.getString("regdate");
			Calendar newcal=Calendar.getInstance();
		...
		rs=st.executeQuery("select regdate from labs");
		while(rs.next())
		{
			ss=rs.getString("regdate");
			Calendar newcal=Calendar.getInstance();
